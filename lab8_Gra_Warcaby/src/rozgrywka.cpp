...
rozgrywka::rozgrywka(){ // konstruktor
  int i,j;
  ...
  for(i=5;i<8;i++) // przejscie po wierszach 6-9
    {
      for(j=0;j<8;j++) // po kolumnach
	{
  ...
  for(i=0;i<8;i++)
   ...
   for(j=0;j<8;j++){
	...
	cout <<"|";
      ...
      cout << "  |" << (i+1) << endl; // numery wierszy
      cout << " ";
      for(w=0;w<8;w++)
	cout << "|_";
      cout << "|" << endl;
    }
  cout << " ";		// wyswietlenie numeracji kolumn
  for(i=0;i<8;i++)
    cout << " _";
  cout << endl;   
  cout << " ";
...
{ 
  ...
  cout << " X - wiersz, Y - kolumna" << endl;    
  cout << " Podaj wspolrzedne pionka do przesunienia (X Y):"  << endl; 
  while(i!=(-1))
    {
      if(i>0)cout <<  "Blad wyboru!" << endl;  
      cin >> wspx_pocz;
      cin >> wspy_pocz;
      wspx_pocz--;
      wspy_pocz--;
  ...
  cout << " Miejsce docelowe: (X Y)" << endl; 
...
void rozgrywka::ruch_komputera(int ktory_gracz){
 int wspx_pocz, wspy_pocz, wspx_kon, wspy_kon;
  int a,b;
  int glebokosc=3;		// zmiana poziomu przeszukiwania

  drz_ocen.ktory_gracz=2;
  drz_ocen.max_min=true;
  for(a=0; a<8; a++){
    for(b=0; b<8; b++){
      drz_ocen.move.plansza[a][b].pawn=board[a][b].pawn;
      drz_ocen.move.plansza[a][b].damka=board[a][b].damka;
    }
  }
  drz_ocen.move.ocena=drz_ocen.minmax(glebokosc);
  drz_ocen.zwroc_wspolrzedne(wspx_pocz, wspy_pocz, wspx_kon, wspy_kon);
